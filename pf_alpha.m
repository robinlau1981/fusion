function [rmsError,rmsError_int1,rmsError_int2,time_elapsed,ESS,state_est,error,alpha] = ...
...
for j=1:no_of_runs
    ...
    tic;
    for t=2:ns,
        fprintf('run = %i / %i : PF-alpha : t = %i / %i  \r',j,no_of_runs,t,ns);
        fprintf('\n')
        
        % PREDICTION STEP:
        % We use the transition prior as proposal.
        yPred=zeros(2,1);  % Store predicted mean values of sensor observations
        for i=1:N
            ...
            yPred(1)=yPred(1)+solve_atan(xparticlePred_pf(3:4,i));
            yPred(2)=yPred(2)+norm(xparticlePred_pf(3:4,i));
        end
        yPred=yPred/N;
        % EVALUTATE Probability of sensor failure
        % likelihoods of sensor functioning well and sensor failure
        lik_well_1=normpdf(y(1,t)-yPred(1),0,sigma_v(1));
        lik_fail_1=1/(2*pi);
        lik_well_2=normpdf(y(2,t)-yPred(2),0,sigma_v(2));
        lik_fail_2=1e-4;
        alpha(1,t,j)=1/2*lik_well_1/(1/2*lik_well_1+1/2*lik_fail_1);
        alpha(2,t,j)=1/2*lik_well_2/(1/2*lik_well_2+1/2*lik_fail_2);
        ...
        for i=1:N
            loglik1= (-1/2*(y(1,t)-yPred_pf(1,i))^2/(sigma_v(1)^2)-log(sigma_v(1)*sqrt(2*pi)))*alpha(1,t,j); 
            loglik2= (-1/2*(y(2,t)-yPred_pf(2,i))^2/(sigma_v(2)^2)-log(sigma_v(2)*sqrt(2*pi)))*alpha(2,t,j); 
            if observation_mode==2
                if t>=idx_start_1 && t<=idx_end_1
